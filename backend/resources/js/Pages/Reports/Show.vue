    ...
    <AdminLayout>
        <div class="animate-fade-in">
            <div class="flex items-center gap-4 mb-8">
                ...
                <div>
                    <h2 class="text-2xl font-bold text-slate-900 tracking-tight">{{ __('Attendance Report') }}</h2>
                    <p class="text-sm text-slate-500 mt-0.5">
                        <span v-if="semester">{{ semester.name }} ({{ semester.academic_year }}) | </span>
                        {{ courseName }} | {{ className }} | {{ subjectName }} | {{ statusLabel }}
                    </p>
                    <p class="text-[12px] text-slate-400 mt-0.5">{{ formatDate(filters?.date_from) }} {{ __('to') }} {{ formatDate(filters?.date_to) }}</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-6 gap-4 mb-8 stagger-children">
                <Card class="animate-fade-in-up">
                    <CardContent class="p-4 text-center">
                        <p class="text-[13px] text-slate-500">{{ __('Total Records') }}</p>
                ...
                <Card class="animate-fade-in-up">
                    <CardContent class="p-4 text-center">
                        <p class="text-[13px] text-slate-500">{{ __('Attendance Rate') }}</p>

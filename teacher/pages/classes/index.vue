<template>
  <div class="animate-fade-in">
    <div class="mb-8">
      <h2 class="text-2xl font-bold text-slate-900 tracking-tight">{{ __('My Classes') }}</h2>
      <p class="text-sm text-slate-500 mt-1">{{ __('Classes assigned to you this semester') }}</p>
    </div>
    ...
    <div v-else class="card overflow-hidden">
      <Empty class="border-0 rounded-none">
        <EmptyHeader>
          ...
          <EmptyTitle>{{ __('No classes assigned') }}</EmptyTitle>
          <EmptyDescription>{{ __('You have no classes assigned for this semester.') }}</EmptyDescription>
        </EmptyHeader>
      </Empty>
    </div>
  </div>

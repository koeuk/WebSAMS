    ...
    <div class="w-full max-w-md animate-fade-in">
      <div class="card p-10" style="box-shadow: 0 20px 60px rgba(30,58,110,0.12), 0 4px 16px rgba(0,0,0,0.06);">
        <div class="text-center mb-8">
          ...
          <p class="text-[13px] text-slate-500 mt-1">Teacher Portal</p>
        </div>

        <div v-if="error" class="mb-4 px-4 py-3 rounded-xl bg-rose-50 border border-rose-200 text-[13px] text-rose-700">
          {{ error }}
        </div>
        ...
        <form @submit.prevent="submit" class="space-y-5">
          <div>
            ...
            <input v-model="email" type="email" required autofocus class="input-modern" placeholder="teacher1@websams.com" />
          </div>
          <div>

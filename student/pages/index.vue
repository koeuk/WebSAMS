    ...
    <div class="flex items-start justify-between mb-7">
      <div>
        <h2 class="text-2xl font-bold text-slate-900 tracking-tight">
          {{ __('dashboard.greeting', { name: firstName }) }}
        </h2>
    ...
    <template v-else-if="dashboard">
        ...
        <div class="card p-6 flex items-center gap-6 lg:col-span-1">
          <div class="relative shrink-0">
            ...
            <div class="absolute inset-0 flex flex-col items-center justify-center">
              ...
              <span class="text-[10px] text-slate-400 mt-0.5 uppercase tracking-wide">{{ __('common.rate') }}</span>
            </div>
          </div>
          <div>
            <p class="text-sm font-semibold text-slate-700 mb-3">{{ __('dashboard.attendanceRate') }}</p>
            <div class="space-y-1.5">
              <div class="flex items-center gap-2 text-[13px]">
                <span class="w-2 h-2 rounded-full bg-emerald-400"></span>
                <span class="text-slate-500">{{ __('common.present') }}</span>
                <span class="ml-auto font-semibold text-slate-800">{{ dashboard.summary?.present }}</span>
              </div>
              <div class="flex items-center gap-2 text-[13px]">
                <span class="w-2 h-2 rounded-full bg-rose-400"></span>
                <span class="text-slate-500">{{ __('common.absent') }}</span>
                <span class="ml-auto font-semibold text-slate-800">{{ dashboard.summary?.absent }}</span>
              </div>
              <div class="flex items-center gap-2 text-[13px]">
                <span class="w-2 h-2 rounded-full bg-amber-400"></span>
                <span class="text-slate-500">{{ __('common.late') }}</span>
              ...
              <div class="flex items-center gap-2 text-[13px] pt-1 border-t border-slate-100">
                ...
                <span class="text-slate-500">{{ __('common.total') }}</span>
        ...
        <div class="lg:col-span-2 grid grid-cols-2 gap-4">
          <div class="card stat-card stat-card-blue p-5 animate-fade-in-up">
            <div class="flex items-center justify-between mb-3">
              <p class="text-[12px] font-semibold text-slate-500 uppercase tracking-wide">{{ __('common.total') }}</p>
          ...
          <div class="card stat-card stat-card-emerald p-5 animate-fade-in-up">
            <div class="flex items-center justify-between mb-3">
              <p class="text-[12px] font-semibold text-slate-500 uppercase tracking-wide">{{ __('common.present') }}</p>
          ...
          <div class="card stat-card stat-card-rose p-5 animate-fade-in-up">
            <div class="flex items-center justify-between mb-3">
              <p class="text-[12px] font-semibold text-slate-500 uppercase tracking-wide">{{ __('common.absent') }}</p>
          ...
          <div class="card stat-card stat-card-amber p-5 animate-fade-in-up">
            <div class="flex items-center justify-between mb-3">
              <p class="text-[12px] font-semibold text-slate-500 uppercase tracking-wide">{{ __('common.late') }}</p>
      ...
      <div class="card overflow-hidden mb-5">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center gap-2">
          ...
          <h3 class="text-sm font-semibold text-slate-900 uppercase tracking-wider">{{ __('dashboard.attendanceBySubject') }}</h3>
        </div>
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead>{{ __('common.subject') }}</TableHead>
              <TableHead class="text-center">{{ __('common.total') }}</TableHead>
              <TableHead class="text-center">{{ __('common.present') }}</TableHead>
              <TableHead class="text-center">{{ __('common.absent') }}</TableHead>
              <TableHead style="min-width:140px;">{{ __('common.rate') }}</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            ...
            <TableRow v-if="!dashboard.bySubject?.length">
              <TableCell colspan="5" class="text-center py-12 text-slate-400">{{ __('dashboard.noAttendanceYet') }}</TableCell>
            </TableRow>
          </TableBody>
        </Table>
      </div>
      ...
      <div class="card overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center gap-2">
          ...
          <h3 class="text-sm font-semibold text-slate-900 uppercase tracking-wider">{{ __('dashboard.myClasses') }}</h3>
        </div>
        <Table>
          <TableHeader>
            <TableRow>
